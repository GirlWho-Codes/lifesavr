...
} from '../../../../components/sections/clique';
...
export async function getServerSideProps(ctx) {
   ...
   const res = await axios.get(
      `${process.env.NEXT_PUBLIC_BASE_URL}/api/v1/admin/view-clique-savings/${id}`,
      {
         headers: {
...
export default function SavingsChallenge({ cliqueData }) {
   ...
   const tabs = [
      {
         heading: 'Savings challenge details',
         component: <PersonalInfo cliqueData={cliqueData} />,
      },
      {
         heading: 'Transactions',
         component: <PersonalSaving cliqueData={cliqueData} />,
      },
   
   ];

   return (
      <Layout title={'Savings Challenge'}>
         <section className='space-y-4'>
            ...
            <div className='start space-x-5'>
               <Image
                  src={
                     cliqueData.profile_phone ? cliqueData.profile_phone : ToyAvatar
                  }
                  alt='lifesaver user'
               />
               <span className='text-xl font-bold'>{cliqueData.clique_savings?.creator?.name}</span>
            </div>
